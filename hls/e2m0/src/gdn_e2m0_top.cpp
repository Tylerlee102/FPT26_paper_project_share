...
e2m0_t unpack_e2m0(state_residual_word_t word, int lane) {
#pragma HLS INLINE
  return word.range(3 * lane + 2, 3 * lane);
}

void pack_e2m0(state_residual_word_t &word, int lane, e2m0_t value) {
#pragma HLS INLINE
  word.range(3 * lane + 2, 3 * lane) = value;
}
...
bool valid_e2m0_code(e2m0_t code) {
#pragma HLS INLINE
  return code != 4;
}
...
#pragma HLS PIPELINE II=1
          ...
          const e2m0_t residual_code = residual[head][row][column];
          valid = valid && valid_e2m1_code(primary_code) &&
                  valid_e2m0_code(residual_code);
...
#pragma HLS PIPELINE II=1
        ...
        if (state_term == 0) {
          ...
          const e2m0_t state_code = unpack_e2m0(
              residual[value_head][row][value_block], value_lane);
          terms[index] = product_e2m1_e2m0(vector_code, state_code);
          exponents[index] =
              decode_scale(vector_scales[vector_term][qk_head][qk_block]) +
...
quantize_fold_primary:
  ...
  const int upper_power = select_e2m0_scale_power(
      residual_m, residual_x, counters);
  const bool lower = choose_lower_e2m0_scale(
      residual_m, residual_x, upper_power);
  int residual_power = lower && upper_power > -127
      ? upper_power - 1
      : upper_power;
...
quantize_fold_residual:
  for (int lane = 0; lane < gdn::BLOCK_SIZE; ++lane) {
#pragma HLS PIPELINE II=1
    const e2m0_t code = quantize_e2m0(
        residual_m[lane], residual_x[lane], residual_power);
    pack_e2m0(residual_word, lane, code);
    any_residual = any_residual || code != 0;
    if (e2m0_exceeds_max(
            residual_m[lane], residual_x[lane], residual_power)) {
      ++counters[COUNTER_E2M0_RESIDUAL_CLIPS];
    }
  }
  if (!any_residual) {
    residual_power = 0;
...
#pragma HLS ARRAY_PARTITION variable=base_exponents complete dim=1
          ...
          base_terms[1] = decode_e2m0(
              unpack_e2m0(residual[head][row][block], lane));
...
#pragma HLS PIPELINE II=1
          ...
          pack_e2m0(residual_word, lane, residual_in[head][row][column]);
        }
        primary[head][row][block] = primary_word;
        residual[head][row][block] = residual_word;
        pack_scale(
...
clear_e2m0_output_heads:
  for (int head = 0; head < gdn::NUM_VALUE_HEADS; ++head) {
  clear_e2m0_output_values:
...
#pragma HLS ARRAY_PARTITION variable=residual_exponents complete dim=1
  ...
  resident_live[sequence_id][layer_id] = old_live + 1;
...
void gdn_e2m0_top(
    ...
    std::uint8_t layer_id,
    std::uint8_t payload_flags,
    const gdn_e2m0::qk_elements_t q_elements,
    const gdn_e2m0::qk_scales_t q_scales,
    const gdn_e2m0::qk_elements_t k_elements,
    const gdn_e2m0::qk_scales_t k_scales,
    const gdn_e2m0::value_elements_t v_elements,
    const gdn_e2m0::value_scales_t v_scales,
    const gdn_e2m0::coefficient_heads_t alpha,
    const gdn_e2m0::coefficient_heads_t beta,
    const gdn_e2m0::state_primary_elements_t state_primary_in,
    const gdn_e2m0::state_scales_t state_primary_scales_in,
    const gdn_e2m0::state_residual_elements_t state_residual_in,
    const gdn_e2m0::state_scales_t state_residual_scales_in,
    const gdn_e2m0::log_key_elements_t log_keys_in,
    const gdn_e2m0::log_key_scales_t log_key_scales_in,
    const gdn_e2m0::log_update_elements_t log_updates_in,
    const gdn_e2m0::log_update_scales_t log_update_scales_in,
    const gdn_e2m0::coefficient_heads_t gamma_in,
    const gdn_e2m0::lambda_t lambda_in,
    gdn_e2m0::live_entries_t live_entries_in,
    gdn_e2m0::output_mantissas_t output_mantissas,
    gdn_e2m0::output_exponents_t output_exponents,
    gdn_e2m0::state_primary_elements_t state_primary_out,
    gdn_e2m0::state_scales_t state_primary_scales_out,
    gdn_e2m0::state_residual_elements_t state_residual_out,
    gdn_e2m0::state_scales_t state_residual_scales_out,
    gdn_e2m0::log_key_elements_t log_keys_out,
    gdn_e2m0::log_key_scales_t log_key_scales_out,
    gdn_e2m0::log_update_elements_t log_updates_out,
    gdn_e2m0::log_update_scales_t log_update_scales_out,
    gdn_e2m0::coefficient_heads_t gamma_out,
    gdn_e2m0::lambda_t lambda_out,
    gdn_e2m0::live_entries_t live_entries_out[1],
    std::uint8_t status_out[1],
    gdn_e2m0::generation_t generation_out[1],
    gdn_e2m0::counters_t command_counters_out,
    gdn_e2m0::counters_t cumulative_counters_out) {
...
#pragma HLS INTERFACE m_axi port=output_exponents offset=slave bundle=gmem9
