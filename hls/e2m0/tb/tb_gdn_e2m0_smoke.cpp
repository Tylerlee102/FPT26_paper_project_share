...
void initialize_payloads() {
  for (int term = 0; term < gdn_e2m0::STACK_DEPTH; ++term) {
    for (int head = 0; head < gdn::NUM_QK_HEADS; ++head) {
      for (int block = 0; block < gdn::QK_BLOCKS; ++block) {
        q_scales[term][head][block] = gdn_e2m0::E8M0_BIAS;
        k_scales[term][head][block] = gdn_e2m0::E8M0_BIAS;
      }
    }
    for (int head = 0; head < gdn::NUM_VALUE_HEADS; ++head) {
      for (int block = 0; block < gdn::VALUE_BLOCKS; ++block) {
        v_scales[term][head][block] = gdn_e2m0::E8M0_BIAS;
      }
    }
  }
  for (int head = 0; head < gdn::NUM_VALUE_HEADS; ++head) {
    alpha[head] = gdn_e2m0::COEFFICIENT_ONE;
    beta[head] = gdn_e2m0::COEFFICIENT_ONE;
    gamma_in[head] = gdn_e2m0::COEFFICIENT_ONE;
    for (int row = 0; row < gdn::KEY_DIM; ++row) {
      for (int block = 0; block < gdn::VALUE_BLOCKS; ++block) {
        state_primary_scales_in[head][row][block] = gdn_e2m0::E8M0_BIAS;
        state_residual_scales_in[head][row][block] = gdn_e2m0::E8M0_BIAS;
      }
    }
  }
  for (int entry = 0; entry < gdn_e2m0::LOG_CAPACITY; ++entry) {
    for (int term = 0; term < gdn_e2m0::STACK_DEPTH; ++term) {
      for (int head = 0; head < gdn::NUM_QK_HEADS; ++head) {
        for (int block = 0; block < gdn::QK_BLOCKS; ++block) {
          log_key_scales_in[entry][term][head][block] = gdn_e2m0::E8M0_BIAS;
        }
      }
      for (int head = 0; head < gdn::NUM_VALUE_HEADS; ++head) {
        for (int block = 0; block < gdn::VALUE_BLOCKS; ++block) {
          log_update_scales_in[entry][term][head][block] = gdn_e2m0::E8M0_BIAS;
        }
      }
    }
  }
...
void invoke(
    ...
    std::uint8_t layer,
    std::uint8_t payload,
    gdn_e2m0::live_entries_t live = 0) {
  gdn_e2m0_top(
      command,
      0,
      layer,
...
int main() {
  ...
  for (int token = 0; token < gdn_e2m0::LOG_CAPACITY; ++token) {
    invoke(gdn::COMMAND_STEP, 0, gdn::PAYLOAD_TOKEN);
    if (!expect_status(gdn::STATUS_OK, "zero step") ||
        generation_out[0] != static_cast<unsigned>(token + 1)) {
      return 1;
    }
    const unsigned expected_live = token + 1 == gdn_e2m0::LOG_CAPACITY
        ? 0u
  ...
  if (cumulative_counters[gdn_e2m0::COUNTER_FOLDS].to_uint64() != 1 ||
      cumulative_counters[gdn_e2m0::COUNTER_COMMITTED_GENERATIONS].to_uint64() !=
          gdn_e2m0::LOG_CAPACITY) {
  ...
  if (!expect_status(gdn::STATUS_OK, "readback") ||
      live_entries_out[0] != 0 || gamma_out[0] != gdn_e2m0::COEFFICIENT_ONE ||
      state_primary_out[0][0][0] != 0 || state_residual_out[0][0][0] != 0 ||
      state_primary_scales_out[0][0][0] != gdn_e2m0::E8M0_BIAS ||
      state_residual_scales_out[0][0][0] != gdn_e2m0::E8M0_BIAS) {
    return 1;
  }
  ...
  std::cout << "PASS: E2M0 resident smoke commands, seven-token fold, snapshot, and layer bounds\n";
  return 0;
}
