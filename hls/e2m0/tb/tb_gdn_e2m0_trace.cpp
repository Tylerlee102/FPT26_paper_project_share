...
bool read_snapshot_input(std::ifstream &input, std::uint8_t &live) {
  return read_u8(
         ...
         read_u8(
             input,
             &log_keys_in[0][0][0][0],
             gdn_e2m0::LOG_CAPACITY * gdn_e2m0::STACK_DEPTH *
         ...
         read_u16(
             input,
             &lambda_in[0][0],
             gdn_e2m0::LOG_CAPACITY * gdn::NUM_VALUE_HEADS) &&
...
bool read_token(std::ifstream &input) {
  return read_u8(
             input,
             &q_elements[0][0][0],
             gdn_e2m0::STACK_DEPTH * gdn::NUM_QK_HEADS * gdn::KEY_DIM) &&
         read_u8(
             input,
             &q_scales[0][0][0],
             gdn_e2m0::STACK_DEPTH * gdn::NUM_QK_HEADS * gdn::QK_BLOCKS) &&
         read_u8(
             input,
             &k_elements[0][0][0],
             gdn_e2m0::STACK_DEPTH * gdn::NUM_QK_HEADS * gdn::KEY_DIM) &&
         ...
         read_u8(
             input,
             &v_elements[0][0][0],
             gdn_e2m0::STACK_DEPTH * gdn::NUM_VALUE_HEADS * gdn::VALUE_DIM) &&
         read_u8(
             input,
             &v_scales[0][0][0],
             gdn_e2m0::STACK_DEPTH * gdn::NUM_VALUE_HEADS * gdn::VALUE_BLOCKS) &&
...
int main(int argc, char **argv) {
  ...
  if (!read_native(input, header, 10) || std::memcmp(magic, "E2M0T01", 7) != 0 ||
      header[0] != 1 || header[1] != 0x01020304u ||
      header[3] != gdn::NUM_QK_HEADS || header[4] != gdn::NUM_VALUE_HEADS ||
      header[5] != gdn::KEY_DIM || header[6] != gdn::VALUE_DIM ||
      header[7] != gdn::BLOCK_SIZE || header[8] != gdn_e2m0::STACK_DEPTH ||
      header[9] != gdn_e2m0::LOG_CAPACITY) {
  ...
  invoke(gdn::COMMAND_LOAD, gdn::PAYLOAD_STATE, initial_live);
  if (status_out[0] != gdn::STATUS_OK || generation_out[0] != 1) {
    std::cerr << "initial LOAD failed\n";
    return 1;
  }
  ...
  for (std::uint32_t token = 0; token < tokens; ++token) {
    ...
    std::uint64_t expected_command[gdn_e2m0::COUNTER_COUNT]{};
    std::uint64_t expected_cumulative[gdn_e2m0::COUNTER_COUNT]{};
    if (!read_scalar(input, expected_status) ||
        !read_scalar(input, expected_generation) ||
        !read_scalar(input, expected_live) ||
        !read_native(input, expected_command, gdn_e2m0::COUNTER_COUNT) ||
        !read_native(input, expected_cumulative, gdn_e2m0::COUNTER_COUNT) ||
    ...
    for (int counter = 0; counter < gdn_e2m0::COUNTER_COUNT; ++counter) {
  ...
  std::cout << "PASS: " << tokens
            << " encoded random-state tokens, exact outputs/counters, and final snapshot\n";
  return 0;
}
