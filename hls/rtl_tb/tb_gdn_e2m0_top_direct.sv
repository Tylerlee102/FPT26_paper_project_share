...
module tb_gdn_e2m0_top_direct;
    ...
    localparam integer COUNTERS = 8;
    localparam integer STATE_ELEMENTS = 32 * 128 * 128;
    localparam integer STATE_SCALES = 32 * 128 * 4;
    localparam integer LOG_KEY_ELEMENTS = 7 * 2 * 16 * 128;
    localparam integer LOG_KEY_SCALES = 7 * 2 * 16 * 4;
    localparam integer LOG_UPDATE_ELEMENTS = 7 * 2 * 32 * 128;
    localparam integer LOG_UPDATE_SCALES = 7 * 2 * 32 * 4;
    localparam integer LAMBDA_ELEMENTS = 7 * 32;
    ...
    `CONNECT_MEM(0, 32, 4224)
    `CONNECT_MEM(1, 32, 4224)
    ...
    function automatic [15:0] mem9_u16(input integer address);
        mem9_u16 = {mem9.mem[address+1], mem9.mem[address]};
    endfunction
    ...
    task automatic run_command(
        ...
        begin
            ...
            @(negedge ap_clk);
            $display(
                "E2M0_DIRECT_RTL command=%0d cycles=%0d status=%0d generation=%0d",
    ...
    task automatic load_token(input integer token);
        ...
        begin
            ...
            for (token_item = 0; token_item < Q_SCALES; token_item = token_item + 1) begin
                mem0.mem[32'h00001000+token_item] = trace_q_scales[token*Q_SCALES+token_item];
                mem1.mem[32'h00001000+token_item] = trace_k_scales[token*Q_SCALES+token_item];
            end
            ...
            for (token_item = 0; token_item < V_SCALES; token_item = token_item + 1)
                mem2.mem[32'h00002000+token_item] = trace_v_scales[token*V_SCALES+token_item];
    ...
    task automatic check_token(input integer token);
        ...
        begin
            ...
            for (token_item = 0; token_item < OUTPUT_ELEMENTS; token_item = token_item + 1) begin
                ...
                if (mem9_u16(32'h00004000+2*token_item) !==
            ...
            $display("E2M0_DIRECT_RTL_TOKEN token=%0d status=%0d generation=%0d live=%0d PASS",
    ...
    task automatic configure_pointers;
        begin
            ...
            set_pointer(9'h01c, 64'h00001000);
            set_pointer(9'h028, 64'h00000000);
            set_pointer(9'h034, 64'h00001000);
            set_pointer(9'h040, 64'h00000000);
            set_pointer(9'h04c, 64'h00002000);
            ...
            set_pointer(9'h0f4, 64'h00004000);
    ...
    initial begin
        ...
        if (token_limit != TRACE_TOKENS) begin
            $display("E2M0_DIRECT_RTL_BENCHMARK PASS tokens=%0d", token_limit);
            $finish;
        end
        ...
        $display(
            "E2M0_DIRECT_RTL_TRACE64 PASS tokens=64 transactions=66 outputs=262144 final_snapshot=PASS");
        $finish;
    end
endmodule
