...
| D2.13 | Reference [4] lacks venue/publisher metadata. | The 2026-08-06 primary-source audit still finds Gupta et al. only as an arXiv preprint. The bibliography explicitly labels that status and supplies authors, title, version, date, class, and DOI rather than inventing a venue. | `paper/corrected/paper.tex`; `docs/evidence/citation_archival_audit_2026_08_06_v4.csv` | PASS |
| D2.14 | Too many references are arXiv-only. | Archival versions replace preprints wherever one was verified. Of 16 entries, eight are archival papers, one is a standard, three are pinned artifacts, and four remain explicitly labeled arXiv preprints with no archival venue metadata on their primary records as of 2026-08-06. | `paper/corrected/paper.tex`; `docs/evidence/citation_archival_audit_2026_08_06_v4.csv`; `docs/evidence/citation_archival_audit_2026_08_06_v4.md`; `tests/test_citation_archival_audit.py` | PASS |

## Reviewer 3

| ID | Concern | Current response and boundary | Primary evidence | Status |
|---|---|---|---|---|
| D3.1 | Explain which applications or model sizes fit on chip. | The selected 36-layer recurrence-state candidate physically fits out of context at 598/960 URAMs and 341/2,016 BRAM tiles. The paper separately states that shell integration, weights, convolution state, attention KV cache, and complete-model residency are not established. | `paper/corrected/paper.tex`; `reports/vivado/corrected/rs2_current/rs2_vivado_summary.json` | PASS |
| D3.2 | Add a memory/performance/accuracy trade-off graph. | A generated three-panel figure juxtaposes logical storage, token-8192 synthetic cosine, and matched HLS STEP cost. It shows that the selected method improves storage and stability but is not Pareto-superior to BF16. | `paper/figures/corrected/rs2_memory_performance_accuracy.pdf`; `paper/figures/corrected/rs2_memory_performance_accuracy_manifest.json` | PASS |
| D3.3 | Quantify four-bit accuracy loss. | Output cosine and recurrent-state relative L2 are reported at 64, 256, 1,024, 4,096, and 8,192 tokens for floating Q/DQ and native encoded MXFP4 against FP32, with BF16, MXFP8-state, and INT4 comparators. | `paper/corrected/tables/rs2_controlled_long.tex`; `paper/figures/corrected/rs2_output_cosine_vs_token.pdf`; `paper/figures/corrected/rs2_state_relative_l2_vs_token.pdf` | PASS |
| D3.4 | Provide an energy-efficiency breakdown. | Matched routed BF16, MXFP8, and RS2/R3 vectorless power is broken down by clocks, logic, signals, BRAM, URAM, and DSP, but service-level board energy requires an attached U55C, XRT platform, xclbin, and telemetry. The hardware audit finds none on this host, so the paper makes no measured energy-efficiency claim. | `paper/corrected/tables/rs2_power_breakdown.tex`; `reports/vivado/corrected/rs2_current/rs2_vivado_summary.json`; `reports/environment/hardware_availability.json`; `paper/corrected/paper.tex` | BLOCKED_EXTERNAL |
| D3.5 | Attribute energy gains to precision reduction. | There is no measured energy gain to attribute. Matched out-of-context BF16, MXFP8, and RS2/R3 routes now exist, but their vectorless estimates at different first-closing clocks are not board energy and cannot establish causal energy savings. | `reports/vivado/baselines/bf16/bf16_vivado_summary.json`; `reports/vivado/baselines/mxfp8/mxfp8_vivado_summary.json`; `reports/vivado/corrected/rs2_current/rs2_vivado_summary.json`; `reports/environment/hardware_availability.json` | BLOCKED_EXTERNAL |
...
| D3.7 | Clarify whether comparisons use full-size/high-precision datapaths. | The controlled HLS table fixes device, dimensions, state layout, pipeline, parallelism, and block size; BF16 uses BF16 state/operands with FP32 reductions, native MXFP4 uses E2M1/E8M0, and native MXFP8 uses E4M3/E8M0. The controlled all-layer routes preserve all 36 state slots for every variant. | `paper/corrected/paper.tex`; `paper/corrected/tables/rs2_hls.tex`; `reports/vivado/baselines/bf16/bf16_vivado_summary.json`; `reports/vivado/baselines/mxfp8/mxfp8_vivado_summary.json`; `reports/vivado/corrected/rs2_current/rs2_vivado_summary.json` | PASS |
| D3.8 | Compare with a modern four-bit GPU or explain why unavailable. | The Windows and WSL hardware audit detects only an RTX 3070 (compute capability 8.6), not a native-FP4 GPU. It also inventories every installed XPFM and XRT management command. A synchronized same-boundary Blackwell-class comparison therefore cannot run on this host, and the paper makes no GPU speed or energy claim. | `reports/environment/hardware_availability.json`; `paper/corrected/paper.tex`; `docs/experimental_protocol.md` | BLOCKED_EXTERNAL |

## Decision Summary

The reviews' novelty, explanation, figure, quantization-contract, reference,
synthetic-stability, and source-level scalability concerns are directly
addressed in source. Matched BF16 and native-MXFP8 HLS/post-route evidence and
short model-derived recurrence traces are complete. Closed-loop real-model
