...
> **Superseded development record.** This document preserves an earlier
> floating-Q/DQ MXFP8-log search and is not the current candidate-selection
> authority. The registered paper candidate is
> `mxfp4_rs2_act_rs2_state_mxfp4rs2_log_r3_q1_15_int32_guard5`; see
...
Development-selected variant (now superseded): `mxfp4_rs2_act_rs2_sparse75_base_b32_mxfp8_log_fixed_b32_r7`.
...
Within this historical sweep, the selected point was the only tested full-8192 candidate that cleared its frozen synthetic checkpoint gate while using fewer logical recurrent-state bytes than uniform MXFP8-B32. The result is retained as search history and is not imported into the current manuscript.
