...
- Extracted the final default 128-token RTL cosim result and used it for default latency extraction.
...
- Latency target: hard floor met (51.472 us/token; target <= 50 us, hard floor <= 100 us).
...
- Speedup vs USC target: hard floor met (1.228x; target >= 2x, hard floor >= 1x).
- Speedup vs H100 target: hard floor met (5.537x; target >= 6x, hard floor >= 4.5x).
