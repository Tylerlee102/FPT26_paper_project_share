...
from scripts.rs2_snapshot_write_contiguous_banks_experiment import generate
from scripts.rs2_snapshot_write_contiguous_banks_report import (
    OUTPUT as ARCHIVE,
    _origins,
)


def test_generator_localizes_snapshot_writes_on_contiguous_bank_parent(
    tmp_path: Path,
) -> None:
    ...
    assert result["parent_transform"]["partition_factor"] == 2
    assert result["parent_transform"]["partition_style"] == "block"
    assert result["parent_transform"]["layers_per_bank"] == 18
    assert result["snapshot_write_transform"] == {"helpers": 1, "replacements": 1}
...
def test_snapshot_contiguous_bank_hls_flow_is_isolated_and_registered() -> None:
    root = Path(__file__).resolve().parents[1]
    csim = (
        root / "hls/rs2/tcl/run_snapshot_write_contiguous_banks_csim.tcl"
    ).read_text()
    csynth = (
        root / "hls/rs2/tcl/run_snapshot_write_contiguous_banks_csynth.tcl"
    ).read_text()
    flow = (root / "scripts/hls_flow.py").read_text()
    makefile = (root / "Makefile").read_text()

    generated = "build/experiments/rs2_snapshot_write_contiguous_banks/gdn_rs2_top.cpp"
    assert "gdn_rs2_snapshot_write_contiguous_banks_trace_hls" in csim
    assert "gdn_rs2_snapshot_write_contiguous_banks_hls" in csynth
    assert generated in csim
    assert generated in csynth
    assert "rs2-snapshot-write-contiguous-banks-csim" in flow
    assert "rs2-snapshot-write-contiguous-banks-csynth" in flow
    assert "hls-rs2-snapshot-write-contiguous-banks-csim" in makefile
    assert "hls-rs2-snapshot-write-contiguous-banks-csynth" in makefile


def test_snapshot_contiguous_bank_vivado_flow_is_isolated_and_matched() -> None:
    root = Path(__file__).resolve().parents[1]
    create = (
        root / "vivado/tcl/create_rs2_snapshot_write_contiguous_banks_project.tcl"
    ).read_text()
    impl = (
        root / "vivado/tcl/run_rs2_snapshot_write_contiguous_banks_impl.tcl"
    ).read_text()
    flow = (root / "scripts/vivado_flow.py").read_text()
    makefile = (root / "Makefile").read_text()

    assert "gdn_rs2_snapshot_write_contiguous_banks_hls/u55c_250mhz/syn/verilog" in create
    ...
    assert "reports/vivado/experiments/rs2_snapshot_write_contiguous_banks_20260810" in impl
    assert "ExploreWithRemap" in impl
    assert "ExtraNetDelay_high" in impl
    assert "AggressiveExplore" in impl
    assert "rs2-snapshot-write-contiguous-banks-impl" in flow
    assert "vivado-rs2-snapshot-write-contiguous-banks-impl" in makefile
    assert "vivado-rs2-snapshot-write-contiguous-banks-report" in makefile
...
def test_snapshot_contiguous_bank_archive_is_complete_and_source_is_immutable() -> None:
    import json
    ...
    assert (
        ...
        == "REJECT_SNAPSHOT_CONTIGUOUS_BANKING_TIMING"
    )
