...
def test_parsers() -> None:
    assert parse_csim(
        "BF16_HLS_CSIM PASS commands=6 bank_classes=2 generation=2\n"
        "CSim done with 0 errors"
    ) == {"commands": 6, "bank_classes": 2, "generation": 2}
...
def test_real_bf16_hls_evidence(tmp_path: Path) -> None:
    ...
    assert report["csynth"]["step_loop_latency_cycles"]["maximum"] == 4_225_856
    assert report["csynth"]["metrics"]["resources"]["LUT"] == 85_350
    assert report["csynth"]["metrics"]["resources"]["BRAM_18K"] == 269
