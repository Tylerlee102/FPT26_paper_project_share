...
class TestVectors(unittest.TestCase):
    def test_synthetic_vectors_are_deterministic(self) -> None:
        a = generate_synthetic_vector(
            0, seed=0xFB72, num_value_heads=2, num_qk_heads=1, head_dim=8
        )
        b = generate_synthetic_vector(
            0, seed=0xFB72, num_value_heads=2, num_qk_heads=1, head_dim=8
        )
    ...
    def test_generated_outputs_match_golden(self) -> None:
        vector = generate_synthetic_vector(
            1, seed=0xFB72, num_value_heads=2, num_qk_heads=1, head_dim=8
        )
    ...
    def test_vector_round_trip_and_manifest(self) -> None:
        vector = generate_synthetic_vector(
            2, seed=0xFB72, num_value_heads=2, num_qk_heads=1, head_dim=8
        )
