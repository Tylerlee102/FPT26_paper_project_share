...
def test_rs2_trace_dimensions_and_terminal_metadata() -> None:
    trace = parse_trace()
    ...
    assert dimensions["log_capacity"] == 3
    assert dimensions["counter_count"] == 7
...
def test_rs2_trace_stream_and_snapshot_lengths() -> None:
    trace = parse_trace()
    dimensions = trace["dimensions"]
    ...
    assert len(trace["expected"]["command_counters"]) == 64 * 7
