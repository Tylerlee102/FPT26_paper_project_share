...
from scripts.verify_e2m0_encoded_stability import verify_manifest
...
def test_controlled_full_dimension_logical_bytes() -> None:
    ...
    assert logical_encoded_state_bytes(config) == 537_744


def test_trace_records_fold_schedule_and_counter_prefix_sums() -> None:
    config = _config(tokens=8, value_heads=4, qk_heads=2, dim=8)
    result = run_encoded_trace(config, initial_state_mode="zero")
    assert len(result.token_rows) == 16
    assert len(result.checkpoint_rows) == 4
    rows = [row for row in result.token_rows if row["variant"] == VARIANT]
    assert [int(row["live_entries"]) for row in rows] == [1, 2, 3, 4, 5, 6, 0, 1]
    assert [int(row["folded"]) for row in rows] == [0, 0, 0, 0, 0, 0, 1, 0]
    assert [int(row["folds"]) for row in rows] == [0, 0, 0, 0, 0, 0, 1, 1]
    running = {
