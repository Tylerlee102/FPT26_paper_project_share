...
from .gdn_bf16 import roundtrip_bf16
...
class WriteLogConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.base_residual_block_fraction <= 1.0:
            raise ValueError("base_residual_block_fraction must be in [0, 1]")
...
def quantize_mxfp4_stack(
    ...
    *,
    block_size: int,
    depth: int,
    residual_block_fraction: float = 1.0,
) -> Array:
    """Reconstruct values as a sum of independently scaled MXFP4 blocks."""

    if depth <= 0:
        raise ValueError("residual-stack depth must be positive")
    if not 0.0 <= residual_block_fraction <= 1.0:
        raise ValueError("residual_block_fraction must be in [0, 1]")
    ...
    for stack_index in range(depth):
        ...
        if stack_index > 0 and residual_block_fraction < 1.0:
            term = _retain_top_residual_blocks(
                term,
                residual,
                block_size=block_size,
                fraction=residual_block_fraction,
            )
...
def _retain_top_residual_blocks(
    term: Array,
    residual: Array,
    *,
    block_size: int,
    ...
    pad_width = [(0, 0)] * term.ndim
    pad_width[-1] = (0, padded_length - length)
    term_padded = np.pad(term, pad_width, mode="constant")
    residual_padded = np.pad(residual, pad_width, mode="constant")
    flat_term = term_padded.reshape(-1, num_blocks, block_size)
    flat_residual = residual_padded.reshape(-1, num_blocks, block_size)
    keep_count = min(num_blocks, int(np.ceil(num_blocks * fraction)))
    if keep_count == 0:
        return np.zeros_like(term)
    scores = np.max(np.abs(flat_residual), axis=-1)
    order = np.argsort(-scores, axis=-1, kind="stable")
    mask = np.zeros_like(scores, dtype=bool)
    np.put_along_axis(mask, order[:, :keep_count], True, axis=1)
    retained = np.where(mask[:, :, None], flat_term, np.float32(0.0))
    return retained.reshape(term_padded.shape)[..., :length].astype(np.float32)
        ...
        *,
        ...
        self.base = (
            ...
            if config.mode == "exact"
                ...
                residual_block_fraction=config.base_residual_block_fraction,
            )
        )
    ...
    def _fold(self, *, rebase: bool = False) -> None:
        ...
        replacement = (
            ...
            if self.config.mode == "exact"
                ...
                residual_block_fraction=self.config.base_residual_block_fraction,
            )
        )
