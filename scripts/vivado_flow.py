...
TCL_BY_STEP = {
    ...
    "rs2-fold-write-partial-banks-impl": Path(
        "vivado/tcl/run_rs2_fold_write_partial_banks_impl.tcl"
    ),
    ...
    "rs2-snapshot-write-unbanked-impl": Path(
        "vivado/tcl/run_rs2_snapshot_write_unbanked_impl.tcl"
    ),
    "rs2-fold-write-contiguous-banks-impl": Path(
        "vivado/tcl/run_rs2_fold_write_contiguous_banks_impl.tcl"
    ),
    "rs2-snapshot-write-contiguous-banks-impl": Path(
        "vivado/tcl/run_rs2_snapshot_write_contiguous_banks_impl.tcl"
    ),
    "rs2-snapshot-write-unbanked-fsm-fanout16-impl": Path(
        "vivado/tcl/run_rs2_snapshot_write_unbanked_fsm_fanout_impl.tcl"
    ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.step.startswith("e2m0-"):
    ...
    elif args.step == "rs2-partial-layer-banks-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "gdn_rs2_partial_layer_banks_hls"
                / "u55c_250mhz"
                / "syn"
                / "verilog"
                / "gdn_rs2_top.v"
            ...
            output=(
                ...
                / "build"
                / "vivado"
                / "rs2_partial_layer_banks_ooc_rtl"
                / "gdn_rs2_top.v"
            ),
        )
    elif args.step == "rs2-fold-write-partial-banks-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "gdn_rs2_fold_write_partial_banks_hls"
                / "u55c_250mhz"
                / "syn"
                / "verilog"
                / "gdn_rs2_top.v"
    ...
    elif args.step == "rs2-snapshot-write-partial-banks-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "gdn_rs2_snapshot_write_partial_banks_hls"
                / "u55c_250mhz"
                / "syn"
                / "verilog"
                / "gdn_rs2_top.v"
    ...
    elif args.step == "rs2-snapshot-write-unbanked-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "gdn_rs2_snapshot_write_unbanked_hls"
                / "u55c_250mhz"
                / "syn"
                / "verilog"
                / "gdn_rs2_top.v"
            ...
            output=(
                ...
                / "build"
                / "vivado"
                / "rs2_snapshot_write_unbanked_ooc_rtl"
                / "gdn_rs2_top.v"
            ),
        )
    elif args.step == "rs2-fold-write-contiguous-banks-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "gdn_rs2_fold_write_contiguous_banks_hls"
                / "u55c_250mhz"
                / "syn"
                / "verilog"
                / "gdn_rs2_top.v"
            ...
            output=(
                ...
                / "build"
                / "vivado"
                / "rs2_fold_write_contiguous_banks_ooc_rtl"
                / "gdn_rs2_top.v"
            ),
        )
    elif args.step == "rs2-snapshot-write-contiguous-banks-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "gdn_rs2_snapshot_write_contiguous_banks_hls"
                / "u55c_250mhz"
                / "syn"
                / "verilog"
                / "gdn_rs2_top.v"
            ...
            output=(
                ...
                / "build"
                / "vivado"
                / "rs2_snapshot_write_contiguous_banks_ooc_rtl"
                / "gdn_rs2_top.v"
            ),
        )
    elif args.step == "rs2-snapshot-write-unbanked-fsm-fanout16-impl":
        prepare_rs2_ooc_rtl(
            source=(
                root
                / "build"
                / "experiments"
                / "rs2_snapshot_write_unbanked_fsm_fanout16"
                / "rtl"
                / "gdn_rs2_top.v"
            ),
            output=(
                ...
                / "build"
                / "vivado"
                / "rs2_snapshot_write_unbanked_fsm_fanout16_ooc_rtl"
                / "gdn_rs2_top.v"
            ),
        )
        ...
        "rs2-snapshot-write-unbanked-impl",
        "rs2-fold-write-contiguous-banks-impl",
        "rs2-snapshot-write-contiguous-banks-impl",
        "rs2-snapshot-write-unbanked-fsm-fanout16-impl",
    }:
        prepare_rs2_ooc_rtl()
