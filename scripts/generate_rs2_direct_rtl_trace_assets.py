"""Convert the frozen RS2/R3 64-token oracle trace into readmemh assets."""
...
def _take(payload: memoryview, offset: int, size: int) -> tuple[bytes, int]:
    stop = offset + size
    if stop > len(payload):
        raise ValueError(f"short RS2 trace at byte {offset}; need {size} bytes")
...
def parse_trace(trace_path: Path = TRACE) -> dict[str, object]:
    ...
    if len(payload) < HEADER.size:
        raise ValueError("RS2 trace is shorter than its header")
    (
        magic,
        version,
        endian,
    ...
    expected = (64, 16, 32, 128, 128, 32, 2, 3)
    actual = (
        tokens,
        qk_heads,
        value_heads,
    ...
    if (magic, version, endian) != (MAGIC, VERSION, ENDIAN_MARKER):
        raise ValueError("RS2 trace identity header mismatch")
    if actual != expected:
        raise ValueError(f"RS2 trace dimensions {actual} do not match {expected}")
    ...
    for _ in range(tokens):
        ...
        for name in ("command_counters", "cumulative_counters"):
            words, offset = _take_words(payload, offset, 7, "Q")
    ...
    if offset != len(payload):
        raise ValueError(f"RS2 trace has {len(payload) - offset} trailing bytes")
    return {
...
def generate_assets(
    ...
    if source_manifest.get("status") != "PASS":
        raise ValueError("source RS2 oracle manifest is not PASS")
    if str(source_manifest.get("trace_sha256", "")).upper() != _sha256(trace_path):
        raise ValueError("source RS2 trace hash does not match its manifest")
