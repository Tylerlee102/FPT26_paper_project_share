"""Generate an isolated RS2 variant with six cyclic layer-state banks."""
...
DEFAULT_OUTPUT = (
    ROOT / "build" / "experiments" / "rs2_partial_layer_banks" / "gdn_rs2_top.cpp"
)
DEFAULT_MANIFEST = (
...
def generate(output: Path, manifest_path: Path) -> dict[str, object]:
    ...
    if transformed == source:
        raise RuntimeError("partial layer-bank transform made no change")
    ...
    manifest = {
        ...
        "behavioral_contract": (
            ...
            "only the primary/residual layer dimension is cyclically partitioned six ways"
        ),
