"""Verify and optionally recompute one encoded RS2 stability artifact."""
...
def verify_manifest(manifest_path: Path, *, recompute: bool) -> dict[str, object]:
    failures: list[str] = []
    ...
    derived_gate = (
        bool(candidate_rows)
        and minimum_cosine >= OUTPUT_COSINE_MINIMUM
    ...
    declared_metrics = manifest.get("gate", {}).get("metrics", {})
    if candidate_rows and (
        float(declared_metrics.get("all_token_output_cosine_min", float("nan")))
        != minimum_cosine
        or float(
            declared_metrics.get("all_token_state_relative_l2_max", float("nan"))
        )
        != maximum_state_error
    ):
        failures.append("declared all-token metrics do not match rows")
    ...
    return {
        ...
        "independent_checks": [
            ...
            "counter prefix sums",
            "all-token quality and arithmetic-event gate",
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = args.output
    if output is None:
        output = manifest.with_name(
            manifest.stem.replace("_manifest", "") + "_verification.json"
        )
