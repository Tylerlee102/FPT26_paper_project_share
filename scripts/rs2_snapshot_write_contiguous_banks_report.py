"""Archive the RS2 snapshot-write plus contiguous-bank experiment."""
...
OUTPUT = ROOT / "reports/vivado/experiments/rs2_snapshot_write_contiguous_banks_20260810"
HLS_ROOT = ROOT / "gdn_rs2_snapshot_write_contiguous_banks_hls/u55c_250mhz"
CSIM_ROOT = ROOT / "gdn_rs2_snapshot_write_contiguous_banks_trace_hls/u55c_250mhz"
MANIFEST = ROOT / "build/experiments/rs2_snapshot_write_contiguous_banks/manifest.json"
OOC_MANIFEST = ROOT / "build/vivado/rs2_snapshot_write_contiguous_banks_ooc_rtl/manifest.json"
IMPL_LOG = ROOT / "reports/vivado/vivado_rs2-snapshot-write-contiguous-banks-impl.log"
...
PARENT = ROOT / "reports/vivado/experiments/rs2_fold_write_contiguous_banks_20260810/summary.json"
UNBANKED_PARENT = ROOT / "reports/vivado/experiments/rs2_fold_write_20260809/summary.json"
...
def _origins(text: str) -> dict[str, int]:
    ...
    for line in text.splitlines():
        ...
        if not path.endswith(("/C", "/CLK")) or line[:1].isspace():
            continue
        if "gmem4_m_axi_U/load_unit" in path:
            label = "gmem4_state_input_load_fifo"
        elif "commit_snapshot_block" in path:
...
def generate_report(output: Path = OUTPUT) -> dict[str, object]:
    ...
    dependencies = (*files.values(), SELECTED_HLS, SELECTED_VIVADO, SELECTED_UTIL, PARENT, UNBANKED_PARENT)
    ...
    unbanked_parent = json.loads(UNBANKED_PARENT.read_text(encoding="utf-8"))
    ...
    source_pass = (
        ...
        and manifest.get("parent_transform", {}).get("partition_factor") == 2
        and manifest.get("parent_transform", {}).get("partition_style") == "block"
    ...
    route_pass = (
        ...
        and "rs2_snapshot_write_contiguous_banks_post_impl.dcp' has been generated" in impl_text
    ...
    unbanked_timing = unbanked_parent["postroute"]["experiment_timing"]
    selected_timing = selected_route["target_clock"]["timing"]
    keys = ("wns_ns", "tns_ns", "setup_failing_endpoints")
    decision = (
        "PROMOTION_CANDIDATE_SNAPSHOT_CONTIGUOUS_BANKING"
        if float(timing["wns_ns"]) >= 0 and float(timing["whs_ns"]) >= 0
        else "RETAIN_SNAPSHOT_CONTIGUOUS_BANKING_CLUE"
        if float(timing["wns_ns"]) > float(parent_timing["wns_ns"])
        else "REJECT_SNAPSHOT_CONTIGUOUS_BANKING_TIMING"
    )
    summary: dict[str, object] = {
        "schema": 1,
        "status": status,
        "experiment": "RS2 snapshot/fold writes with two contiguous layer banks",
        ...
        "hls": {"selected": selected_hls, "experiment": hls},
        "postroute": {
            "selected_timing": selected_timing,
            "contiguous_bank_parent_timing": parent_timing,
            "unbanked_fold_write_parent_timing": unbanked_timing,
            "experiment_timing": timing,
            "delta_experiment_minus_selected": _delta(timing, selected_timing, keys),
            "delta_experiment_minus_contiguous_bank_parent": _delta(timing, parent_timing, keys),
            "delta_experiment_minus_unbanked_fold_write_parent": _delta(timing, unbanked_timing, keys),
    ...
    if status != "PASS":
        raise RuntimeError("refusing to archive invalid snapshot contiguous-bank evidence")
