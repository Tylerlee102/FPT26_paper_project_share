"""Generate an OOC top whose kernel clock is buffered before RTL synthesis."""
...
DEFAULT_SOURCE = (
    ROOT / "gdn_e2m0_hls" / "u55c_250mhz" / "syn" / "verilog" / "gdn_e2m0_top.v"
)
DEFAULT_OUTPUT = ROOT / "build" / "vivado" / "e2m0_ooc_rtl" / "gdn_e2m0_top.v"
...
def transform_top(source: str) -> str:
    ...
    if "module gdn_e2m0_top (" not in source:
        raise ValueError("expected gdn_e2m0_top module")

    prefix, body = source.split(INPUT_DECLARATION, maxsplit=1)
    body = re.sub(r"(?<!\.)\bap_clk\b", "ooc_ap_clk_global", body)
    body = body.replace(
        INSERTION_MARKER,
        f"\n{CLOCK_BUFFER}{INSERTION_MARKER}",
        1,
    )
