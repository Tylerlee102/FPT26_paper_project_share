"""Archive the isolated RS2 fold-write contiguous-bank experiment."""
...
OUTPUT = ROOT / "reports/vivado/experiments/rs2_fold_write_contiguous_banks_20260810"
HLS_ROOT = ROOT / "gdn_rs2_fold_write_contiguous_banks_hls/u55c_250mhz"
CSIM_ROOT = ROOT / "gdn_rs2_fold_write_contiguous_banks_trace_hls/u55c_250mhz"
MANIFEST = ROOT / "build/experiments/rs2_fold_write_contiguous_banks/manifest.json"
OOC_MANIFEST = ROOT / "build/vivado/rs2_fold_write_contiguous_banks_ooc_rtl/manifest.json"
IMPL_LOG = ROOT / "reports/vivado/vivado_rs2-fold-write-contiguous-banks-impl.log"
...
def generate_report(output: Path = OUTPUT) -> dict[str, object]:
    ...
    source_pass = (
        ...
        and manifest.get("partition_factor") == 2
        and manifest.get("partition_style") == "block"
        and len(manifest.get("partition_insertions", [])) == 2
    ...
    route_pass = (
        ...
        and "rs2_fold_write_contiguous_banks_post_impl.dcp' has been generated" in impl_text
    ...
    decision = (
        "PROMOTION_CANDIDATE_CONTIGUOUS_BANKING"
        if float(timing["wns_ns"]) >= 0 and float(timing["whs_ns"]) >= 0
        else "RETAIN_CONTIGUOUS_BANKING_CLUE"
        if float(timing["wns_ns"]) > float(parent_timing["wns_ns"])
        else "REJECT_CONTIGUOUS_BANKING_TIMING"
    )
    ...
    if status != "PASS":
        raise RuntimeError("refusing to archive invalid contiguous-bank evidence")
